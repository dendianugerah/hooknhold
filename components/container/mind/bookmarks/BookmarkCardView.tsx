...
import { ChevronRight, Loader2, Plus, X, GripVertical } from "lucide-react";
...
export function BookmarkCardView({
  ...
  const { attributes, listeners, setNodeRef } = useDraggable({
    id: bookmark.id,
    data: { type: "bookmark", title: bookmark.data.title },
  });
    ...
    <Card key={bookmark.id}>
      <CardContent className="p-4 group relative">
        <div
          ...
          {...attributes}
          {...listeners}
          className="absolute top-2 left-2 cursor-move p-2 rounded-full bg-gray-100 opacity-0 group-hover:opacity-100 transition-opacity hover:bg-gray-200"
        >
              ...
              <Popover open={isPopoverOpen} onOpenChange={setIsPopoverOpen}>
                ...
                <PopoverContent className="flex" align="start">
                  <div className="flex items-center space-x-4 justify-between">
                    ...
                    >
                      {createTagInBookmark.isLoading ? "Saving..." : "Save tag"}
                    </Button>
                  </div>
                </PopoverContent>
              </Popover>
